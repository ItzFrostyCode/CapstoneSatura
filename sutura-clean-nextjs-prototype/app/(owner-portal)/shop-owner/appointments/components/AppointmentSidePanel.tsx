    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-[500px] bg-white z-[201] shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-500 flex flex-col">
        ...
        <div className="p-8 space-y-10">
          ...
          {/* Section 2: Bulk Order / Purpose Context */}
          {appointment.purpose === 'Bulk Order' ? (
            <section className="space-y-4 animate-in slide-in-from-top-4 duration-500">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                  <Box size={14} className="text-emerald-500" /> Bulk Order Breakdown
                </h4>
                <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 text-[9px] font-black uppercase tracking-tighter rounded-md border border-emerald-100">
                  {appointment.estimatedQuantity || '0'} Units
                </span>
              </div>
              
              <div className="p-5 rounded-2xl bg-emerald-50/30 border border-emerald-100/50 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[14px] font-black text-slate-900">{appointment.orgName || 'N/A'}</p>
                    <p className="text-[10px] font-bold text-slate-500 uppercase">Organization / Team Name</p>
                  </div>
                </div>

                {/* Member Roster Table */}
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Member List & Sizes</label>
                  <div className="bg-white rounded-xl border border-emerald-100 overflow-hidden shadow-sm">
                    <div className="max-h-[200px] overflow-y-auto custom-scrollbar">
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="bg-emerald-50/50 border-b border-emerald-100 text-[9px] font-black text-emerald-700 uppercase tracking-tight">
                        ...
                        <tbody className="divide-y divide-emerald-50">
                          {appointment.personnel && appointment.personnel.length > 0 ? (
                            appointment.personnel.map((person, idx) => (
                              <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                <td className="px-3 py-2">
                                  <p className="text-[12px] font-bold text-slate-700">{person.name}</p>
                                  {person.type === 'Custom' && (
                                    <p className="text-[8px] text-emerald-600 font-medium uppercase tracking-tight">
                                      Bespoke Measurements Added
                                    </p>
                                  )}
                                </td>
                                <td className="px-3 py-2">
                                  <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded ${person.type === 'Custom' ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-100 text-slate-500'}`}>
                                    {person.type}
                                  </span>
                                </td>
                            ...
                            <tr>
                              <td colSpan={3} className="px-3 py-4 text-center text-[11px] text-slate-400 italic">No members listed yet</td>
                            </tr>
                          )}
                        </tbody>
                      </table>
              ...
              {/* Inspiration Section */}
              {appointment.inspiration && appointment.inspiration.length > 0 && (
                <div className="space-y-2 pt-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Inspiration & References</label>
                  <div className="flex flex-wrap gap-2">
                    {appointment.inspiration.map((insp, i) => (
                      insp.link && (
                        <a 
                          key={i} 
                          href={insp.link} 
                          ...
                          className="flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-bold text-slate-600 hover:text-indigo-600 hover:border-indigo-100 transition-all"
                        >
                          <ExternalLink size={12} /> View Inspiration {i + 1}
                        </a>
                      )
                    ))}
                  </div>
            ...
            /* Section 2: Standard Customer Profile Context */
            <section className="space-y-6">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
