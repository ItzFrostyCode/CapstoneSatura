...
export default function CustomersPage() {
  ...
  // Form States
  const [customerForm, setCustomerForm] = useState<CustomerForm>({ name: '', email: '', phone: '', address: '', gender: 'Male', source: 'WALKIN', type: 'Regular' });
  ...
  const [aptForm, setAptForm] = useState<Partial<Appointment>>({ 
    type: 'APPOINTMENT', status: 'Scheduled', duration: 60 
  });
  ...
  const handleCreateCustomer = () => {
    ...
    setCustomerForm({ name: '', email: '', phone: '', address: '', gender: 'Male', source: 'WALKIN', type: 'Regular' });
  ...
  const handleScheduleAppointment = () => {
    ...
    const newApt: Appointment = {
      ...
      type: aptForm.type || 'APPOINTMENT',
      category: aptForm.type || 'Appointment',
      date: aptForm.date,
    ...
    setAptForm({ type: 'APPOINTMENT', status: 'Scheduled', duration: 60 });
    ...
    <div className="space-y-6 animate-in fade-in duration-700 max-w-[1600px] mx-auto pb-20">
      ...
      {!selectedCustomerId ? (
        ...
        <CustomerProfile 
          ...
          onNewOrder={() => router.push(`/shop-owner/orders?customerId=${selectedCustomerId}`)}
