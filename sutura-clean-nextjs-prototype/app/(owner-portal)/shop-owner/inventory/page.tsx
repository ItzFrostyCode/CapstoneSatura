    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-[1450px] mx-auto pb-20">
      <main className="max-w-[1450px] mx-auto px-10 pt-8 space-y-8">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { label: 'Raw Materials', val: materials.length, status: 'Available SKUs', color: '#0f172a' },
            { label: 'Finished Units', val: finishedGoods.reduce((sum, i) => sum + (i.stock || 0), 0), status: 'Ready for Release', color: '#3b82f6' },
            { label: 'Low Stock', val: inventory.filter(i => getStatus(i) === 'Low Stock').length, status: 'Watch List', color: '#334155' },
            { label: 'Out of Stock', val: inventory.filter(i => getStatus(i) === 'Out of Stock').length, status: 'Critical', color: '#e11d48' },
          ].map((stat, i) => (
