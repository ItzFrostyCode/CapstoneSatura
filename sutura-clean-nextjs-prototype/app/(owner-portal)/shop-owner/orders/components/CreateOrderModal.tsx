...
export default function CreateOrderModal({ isOpen, onClose }: CreateOrderModalProps) {
  ...
  const [formData, setFormData] = useState<OrderFormData>(() => ({
    orderType: 'CUSTOM_TAILORING' as OrderType,
    ...
    baseSize: 'M',
    customerCategory: 'REGULAR' as CustomerCategory,
    fabricSource: 'SHOP_PROVIDED' as FabricSource
  }));
  ...
  const totalQuantity = useMemo(() => {
    if (formData.orderType === 'BULK_ORDER' && formData.bulkSizingStrategy === 'STANDARD') {
  ...
  const financials = useMemo(() => {
    ...
    if (formData.orderType === 'REPAIR_ALTERATIONS') {
  ...
  const handleFinalSubmit = () => {
    ...
    const itemUnitPrice = isReadyMade ? (selectedProduct?.unit_price || 0) : (formData.orderType === 'REPAIR_ALTERATIONS' ? 0 : (selectedTemplate?.base_price || 0));
    const itemGarmentName = isReadyMade ? (selectedProduct?.item_name || 'Product') : (formData.orderType === 'REPAIR_ALTERATIONS' ? formData.alterationDetails.itemDescription : (selectedTemplate?.name || 'Custom'));
    ...
    const mainOrder = {
      ...
      alteration_details: formData.orderType === 'REPAIR_ALTERATIONS' ? formData.alterationDetails : undefined,
      items: [{ 
        id: `ITEM-${Date.now()}`, 
        ...
        size: formData.baseSize
      }],
      tasks: isReadyMade ? [] : (formData.orderType === 'REPAIR_ALTERATIONS' ? formData.alterationDetails.tasks.map(t => t.title) : (selectedTemplate?.default_tasks || [])).map((t: string, idx: number) => ({ 
        id: `T-${idx + 1}`, 
        job_order_id: newOrderId,
        title: t, 
    ...
    } else if (!isReadyMade && formData.orderType !== 'REPAIR_ALTERATIONS' && !formData.isCustomerProvidedFabric && selectedTemplate) {
  ...
  const nextStep = () => {
    ...
    } else if (formData.orderType === 'BULK_ORDER') {
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-0 md:p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-[800px] h-full md:h-[90vh] md:rounded-[32px] rounded-none shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          ...
          {step === 1 && (
            formData.orderType === 'CUSTOM_TAILORING' || formData.orderType === 'REPAIR_ALTERATIONS' || formData.orderType === 'BULK_ORDER'
          ...
          {step === 2.5 && formData.orderType === 'BULK_ORDER' && (
          ...
          {step === 3 && (
            formData.orderType === 'REPAIR_ALTERATIONS'
...
function getStepTitle(step: number, orderType: OrderType) {
  const titles: Record<number, string> = {
    1: orderType === 'BULK_ORDER' ? 'Customer (Contact)' : 'Product Catalog',
    1.5: 'Select Variant',
    2: orderType === 'BULK_ORDER' ? 'Organization' : (orderType === 'REPAIR_ALTERATIONS' ? 'Garment Intake' : 'Measurements'),
    2.5: 'Personnel Sizing',
    3: orderType === 'REPAIR_ALTERATIONS' ? 'Scope of Request' : 'Template Selection',
    4: orderType === 'REPAIR_ALTERATIONS' ? 'Adjustments' : 'Quantity',
    5: orderType === 'REPAIR_ALTERATIONS' ? 'Materials' : 'Fabric Analysis',
    6: orderType === 'REPAIR_ALTERATIONS' ? 'Service Pricing' : 'Design & Assets',
    7: 'Payment & Deadline',
