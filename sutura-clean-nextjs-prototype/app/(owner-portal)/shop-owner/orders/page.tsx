...
const WORKSHOP_STAGES: Array<{ key: WorkshopStage; label: string; color: string; bg: string; orderStatus: string }> = [
  { key: "ORDER_INTAKE",      label: "Intake",           color: "#1C1917",  bg: "#F0EDE8",    orderStatus: "PENDING_QUOTE" },
  { key: "MEASURING",         label: "Measuring",        color: "#1E3A1F",   bg: "#FAF8F5",     orderStatus: "READY_FOR_FITTING" },
  { key: "MATERIAL_SOURCING", label: "Materials",        color: "#C9A84C",  bg: "#FAF8F5",    orderStatus: "IN_PRODUCTION" },
  { key: "CUTTING",           label: "Cutting",          color: "#1E3A1F", bg: "#FAF8F5",   orderStatus: "IN_PRODUCTION" },
  { key: "SEWING",            label: "Sewing",           color: "#1E3A1F", bg: "#FAF8F5",   orderStatus: "IN_PRODUCTION" },
  { key: "FIRST_FITTING",     label: "Fitting",          color: "#C9A84C", bg: "#FAF8F5",   orderStatus: "READY_FOR_FITTING" },
  { key: "ALTERATIONS",       label: "Alterations",      color: "#B45309",   bg: "#FAF8F5",     orderStatus: "ALTERATIONS" },
  { key: "FINISHING",         label: "Finishing",        color: "#1E3A1F",   bg: "#FAF8F5",     orderStatus: "IN_PRODUCTION" },
...
export default function JobOrdersPage() {
  ...
  useEffect(() => {
    if (searchParams.get('new') === 'true') {
      const timer = setTimeout(() => setIsCreateModalOpen(true), 100);
      ...
      <main className="max-w-[1450px] mx-auto px-10 pt-8 space-y-8">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label: 'Ready for Release', val: jobOrders.filter(o => resolveOrderState(o).productionStage === 'READY_FOR_PICKUP').length, status: 'Storefront', color: '#2D5016' },
                      ...
                      >
                        ...
                        <td className="px-10 py-6">
                          <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest border border-slate-200 px-3 py-1 rounded-lg bg-slate-50">
                            {order.order_type === 'CUSTOM_TAILORING' ? 'Bespoke Workshop' : order.order_type === 'BULK_ORDER' ? 'Bulk Production' : 'Alteration Hub'}
                          </span>
                        </td>
                        <td className="px-10 py-6 text-[15px] font-bold text-slate-900 font-sans">
                          {order.order_type === 'REPAIR_ALTERATIONS' 
        ...
        <div className="space-y-6">
          <div className="px-10 py-4 bg-slate-900 rounded-[24px] text-white flex items-center justify-between shadow-xl shadow-slate-200">
             <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center">
                   <Activity size={20} className="text-indigo-400" />
                </div>
                <div>
                   <h2 className="text-[16px] font-black uppercase tracking-widest">Workshop Pulse</h2>
                   <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tight">Real-time status of every bespoke creation</p>
                </div>
             </div>
             <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">Live Production Feed</span>
             </div>
          </div>

          <div className="flex gap-6 overflow-x-auto pb-10 custom-scrollbar min-h-[600px] px-2">
          {WORKSHOP_STAGES.map((stage, i) => {
            const stageOrders = jobOrders.filter((o) => o.status === stage.orderStatus);
            return (
              <div key={i} className="flex flex-col gap-6 min-w-[320px] w-[320px]">
                <div className="flex items-center justify-between px-4">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full" style={{ background: stage.color }} />
                    <h3 className="text-[13px] font-bold text-slate-900 uppercase tracking-widest">{stage.label}</h3>
                  </div>
                  <span className="bg-slate-100 text-slate-400 px-2.5 py-1 rounded-full text-[11px] font-black">
                    {stageOrders.length}
                  </span>
                </div>

                <div className="flex flex-col gap-4 min-h-[500px] bg-slate-50/30 rounded-[32px] p-3 border border-dashed border-slate-200">
                  {stageOrders.map((order) => (
                    <div
                      key={order.id}
                      className="bg-white border border-slate-200 p-6 rounded-[24px] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer"
                      onClick={() => {
                        setSelectedOrder(order);
                        setReleaseChecklist({ fitting: false, packaging: false });
                        setPaymentAmount(resolveOrderState(order).balance);
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <span
                          className={`text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${getPriorityColor(
                            order.priority
                          )}`}
                        >
                          {order.priority}
                      ...
                      <h4 className="text-[16px] font-bold text-slate-900 leading-tight mb-2 font-sans">
                        {order.items?.[0]?.garment_name || 'Custom Garment'}
                      </h4>
                      <p className="text-[12px] text-slate-500 font-medium mb-5">{customers.find(c => c.id === order.customer_id)?.name || 'Unknown'}</p>

                      <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                        <div className="flex items-center gap-2 text-[11px] font-bold text-slate-400">
                          <Clock size={14} />
                          <span>
                            {order.due_date ? new Date(order.due_date).toLocaleDateString('en-PH', { month: 'short', day: 'numeric' }) : 'N/A'}
                          </span>
                        </div>
                  ...
                  {stageOrders.length === 0 && (
                    <div className="h-32 flex flex-col items-center justify-center text-slate-300 opacity-50">
                      <Layers size={24} className="mb-2" />
                      <span className="text-[10px] font-black uppercase tracking-widest">Station Empty</span>
                    </div>
                  )}
                </div>
              </div>
