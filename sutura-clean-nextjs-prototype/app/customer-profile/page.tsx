...
export default function CustomerProfilePage() {
  ...
  const tabs = [
    { id: 'account', name: 'Customer Portal', icon: <User size={18} /> },
  ...
  return (
    <div className="bg-[#FAF8F5] min-h-screen text-slate-900 overflow-x-hidden font-poppins">
      
      {/* ===== 3-TIER SHOPEE-STYLE HEADER (From Homepage) ===== */}
      <header className="fixed top-0 w-full z-[1000]">
        {/* TIER 1 — UTILITY BAR */}
        <div className="bg-slate-900 text-white text-[11px] font-bold transition-colors">
          <div className="max-w-[1400px] mx-auto px-6 md:px-12 h-9 flex items-center justify-between">
            ...
            <div className="flex items-center gap-6 text-white/70">
                ...
                {showNotifications && (
                  <div className="absolute top-6 right-0 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 p-4 text-slate-900 animate-in fade-in slide-in-from-top-2 duration-300 z-[2000]">
                    <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-50">
                      ...
                      <span className="text-[9px] font-bold text-blue-600 cursor-pointer">Mark all as read</span>
                    </div>
                    <div className="flex gap-3 items-start p-2 bg-slate-50 rounded-xl">
              ...
              {currentUser ? (
                ...
                >
                  <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden border-2 border-white/50 shadow-sm">
                      <img src={currentUser.avatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=John&backgroundColor=b6e3f4"} alt="Avatar" className="w-full h-full object-cover" />
                  </div>
                  ...
                  {showProfileMenu && (
                    <div className="absolute top-8 right-0 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden text-slate-900 animate-in fade-in slide-in-from-top-2 duration-300 z-[2000]">
                      <div className="p-2">
                        <Link 
                          href={currentUser.role === 'CUSTOMER' ? '/customer-profile' : '/shop-owner/settings'} 
                        ...
                        >
                          ...
                          <span className="text-xs font-bold text-slate-700">
                            {currentUser.role === 'CUSTOMER' ? 'Profile Settings' : 'Customer Portal'}
                          </span>
                        </Link>
                        
                        <Link 
                          href={currentUser.role === 'CUSTOMER' ? '/customer/dashboard' : '/shop-owner/dashboard'} 
                          className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-all group"
                        >
                          <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center group-hover:bg-slate-900 transition-all">
                            <Store size={16} className="text-slate-600 group-hover:text-white" />
                          </div>
        ...
        <div className={`bg-slate-900 transition-all duration-300 ${scrolled ? "py-3 shadow-xl" : "py-4"}`}>
          <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex items-center gap-8">
            ...
            <div className="flex-1 flex max-w-[700px]">
              <div className="flex-1 flex items-center bg-white rounded-l-2xl overflow-hidden">
                ...
                <input
                  type="text"
                  placeholder="Search For Shop Owners or garment types..."
              ...
              <button className="bg-slate-200 hover:bg-slate-300 text-slate-900 px-6 rounded-r-2xl font-black transition-all active:scale-95">
                <Search size={20}/>
              </button>
            </div>
          </div>
        ...
        <div 
          className="border-t transition-colors bg-slate-800"
          style={{ borderTopColor: 'rgba(255,255,255,0.1)' }}
        >
          <div className="max-w-[1400px] mx-auto px-6 md:px-12 h-9 flex items-center gap-8 overflow-x-auto scrollbar-hide">
            {["🎩 Custom Barong","🕴️ Bespoke Suits","👰 Bridal Couture","👗 Filipiniana","👟 Streetwear","🏢 Corporate Uniforms","✦ PREMIUM Tailoring Shops"].map((item,i) => (
      ...
      <main className="pt-44 pb-24 max-w-[1400px] mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <h1 className="text-[36px] font-bold text-slate-900 tracking-tight leading-none">Profile Settings</h1>
            <p className="text-[14px] text-slate-500 mt-3">Manage your personal details, order updates, and exclusive vouchers.</p>
          </div>
          
          <div className="flex items-center gap-3">
            <button 
              className="h-12 px-8 bg-slate-900 text-white rounded-xl flex items-center gap-3 text-[14px] font-bold shadow-lg hover:shadow-slate-900/20 hover:bg-slate-800 transition-all active:scale-95 group" 
              onClick={handleSave}
            >
        ...
        <div className="pb-20">
              ...
              <div className="p-8 flex flex-col-reverse md:flex-row gap-12">
                ...
                <div className="flex-1 space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
                    ...
                    <div className="flex-1">
                      <input type="text" defaultValue="johnclock_25" className="w-full h-10 px-4 border border-slate-300 rounded-md text-[14px] text-slate-800 focus:border-slate-900 focus:ring-1 focus:ring-slate-900 outline-none transition-all" />
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
                    ...
                    <div className="flex-1">
                      <input type="text" defaultValue={currentUser?.name || ''} className="w-full h-10 px-4 border border-slate-300 rounded-md text-[14px] text-slate-800 focus:border-slate-900 focus:ring-1 focus:ring-slate-900 outline-none transition-all" />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
                    ...
                    <div className="flex-1 flex items-center gap-3">
                      ...
                      <button className="text-[13px] text-blue-600 font-semibold hover:underline">Change</button>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
                    ...
                    <div className="flex-1 flex items-center gap-6">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer group">
                        <div className="w-4 h-4 rounded-full border-2 border-slate-300 flex items-center justify-center group-hover:border-slate-500 transition-colors"></div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-6 pt-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-8">
                        <button 
                          onClick={handleSave}
                          className="h-10 px-6 bg-slate-900 hover:bg-slate-800 text-white text-[14px] font-bold rounded shadow-sm transition-colors active:scale-95"
                        >
                          Save
                        </button>
                      </div>
                ...
                <div className="md:w-[280px] flex flex-col items-center md:border-l border-slate-100 md:pl-12 pt-4">
                  <div className="w-28 h-28 rounded-full overflow-hidden bg-slate-100 border border-slate-200 mb-6">
                    <img src={currentUser?.avatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=John&backgroundColor=b6e3f4"} alt="Profile" className="w-full h-full object-cover" />
                  </div>
          ...
          {activeTab === 'notifications' && (
            <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm max-w-4xl animate-in fade-in slide-in-from-bottom-2">
              <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><Bell size={20} className="text-slate-900"/> Order & System Updates</h2>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className={`p-4 rounded-xl border ${notif.read ? 'bg-slate-50 border-slate-100' : 'bg-blue-50/50 border-blue-100'} flex gap-4 items-start transition-all hover:shadow-md`}>
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${notif.read ? 'bg-slate-200 text-slate-500' : 'bg-slate-900 text-white'}`}>
                      <Bell size={18} />
                    </div>
                    <div className="flex-1">
                    ...
                    {!notif.read && <div className="w-2.5 h-2.5 bg-blue-500 rounded-full mt-2" /> }
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'vouchers' && (
            <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm max-w-4xl animate-in fade-in slide-in-from-bottom-2">
              <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2"><Ticket size={20} className="text-slate-900"/> My Exclusive Vouchers</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={i} className="relative border border-slate-100 rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-md hover:-translate-y-1 transition-all group cursor-pointer">
                    <div className="absolute left-0 top-0 bottom-0 w-3 bg-slate-900" />
                    <div className="p-5 pl-8">
                      <div className="inline-block px-2 py-1 bg-slate-50 text-slate-700 text-[10px] font-black uppercase tracking-widest rounded mb-3">
                        {voucher.type}
                      </div>
                      ...
                      <div className="mt-4 flex items-center justify-between pt-4 border-t border-slate-100">
                        ...
                        <button className="text-[12px] font-black text-slate-900 group-hover:text-slate-950">Use Now</button>
                      </div>
                    </div>
                    <div className="absolute left-3 top-0 bottom-0 w-[1px] border-l-2 border-dashed border-slate-100" />
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-16">
            ...
            <div>
              ...
              <ul className="space-y-4 text-slate-400 font-bold text-sm">
                {["Find an Workshop","Fabric Catalog","Measurement Guide"].map(l=><li key={l} className="hover:text-slate-300 transition-colors cursor-pointer">{l}</li>)}
              </ul>
            </div>
            <div>
              <h4 className="text-white font-black uppercase tracking-widest text-[11px] mb-7">For Partners</h4>
              <ul className="space-y-4 text-slate-400 font-bold text-sm">
                {["Sutura For Shop Owners","ERP Training","Partner Program"].map(l=><li key={l} className="hover:text-slate-300 transition-colors cursor-pointer">{l}</li>)}
              </ul>
            </div>
            <div className="bg-white/5 p-7 rounded-[32px] border border-white/10">
              ...
              <p className="text-slate-400 text-sm mb-5 font-medium">Get notified of new Tailoring Shops.</p>
              <input type="text" placeholder="Your email..." className="w-full bg-slate-800 rounded-xl px-5 py-3 text-white font-bold outline-none ring-1 ring-white/10 focus:ring-slate-400 transition-all text-sm"/>
            </div>
          </div>
